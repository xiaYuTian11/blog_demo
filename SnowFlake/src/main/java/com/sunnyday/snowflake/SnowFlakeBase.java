 ...
 * 2) 41位，用来记录时间戳（毫秒）。41位可以表示2^41数字，如果只用来表示正整数（计算机中正数包含0），可以表示的数值范围是：0 至 2^41−1，
 ...
 * 4) 12位，序列号，用来记录同毫秒内产生的不同id。12位（bit）可以表示的最大正整数是2^12=4096，即可以用0、1、2、3、....4095这4096个数字，
 * 来表示同一机器同一时间截（毫秒)内产生的4096个ID序号由于在Java中64bit的整数是long类型，所以在Java中SnowFlake算法生成的id就是long来存储的。
 * </>
 *
 * @author TMW
 * @date 2020/10/16 10:16
 */
public class SnowFlakeBase {
    /**
    ...
    public SnowFlakeBase(long machineId, long datacenterId) {
